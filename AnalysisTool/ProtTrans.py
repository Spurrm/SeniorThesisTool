...
myFile_str = input("  Enter File :") #Enter file name to read from
...
for record in SeqIO.parse(myFile_str,'fasta'):
    ...
    PROTfromRNA_strA = RNAfromDNA_str.translate()
    ...
    #DNAfromRNA_str  = seq.back_transcribe(RNAfromDNA_str) # gives DNA sequence from the RNA conversion
    PROTfromRNA_strB = RNAfromDNA_str.translate() #translating RNA into DNA
...
for i in range(len(seq_a)):
    ...
    if seq_a[i] == seq_b[i]:
        print ("\n Sequences are the same at position :", i)
        print ("   seqA_str[i] = ",PROTfromRNA_strA[i])
        print ("   seqB_str[i] = ",PROTfromRNA_strB[i])
